...
To determine this, you must call a function IsCool() with the string. 
If IsCool() returns 'yup' (or another positive response), then the string is cool, and you must call the function DoCoolThing() with the string and return "was cool: X" where X is the return value of DoCoolThing().
 ...
 */
    [CallableFunction("Returns a string indicating whether the input string is cool or not cool. The input string is cool only if return value is positive (eg 'yes' or 'yup', etc)")]
    public async Task<string> IsCool(IsCoolParams isCoolParams)
    {
        return "sure is cool!";
    }
    
    [CallableFunction("Does the cool thing with the input string. Call only if the string is cool")]
