...
namespace AiLib.Tests
{
	public class CompletionEndpointTests
	{
		...
		public void GetBasicCompletion()
		{
			...
			var results = api.Completions.CreateCompletionsAsync(new CompletionRequest("One Two Three Four Five Six Seven Eight Nine One Two Three Four Five Six Seven Eight", model: Model.Model.CurieText, temperature: 0.1, max_tokens: 5)).Result;
		...
		public async Task CreateCompletionAsync_MultipleParamShouldReturnTheSameDataAsSingleParamVersion()
		{
			...
			var resultsMultipleParams = await api.Completions.CreateCompletionAsync(
				r.Prompt, Model.Model.DefaultModel, r.MaxTokens, r.Temperature, r.TopP, r.NumChoicesPerPrompt, r.PresencePenalty,
				r.FrequencyPenalty,
				null, r.Echo);
